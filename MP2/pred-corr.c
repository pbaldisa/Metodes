...
Point predictor(Point initial, Point points[4], int* stop);
Point corrector(Point initial, Point centre, Point X0, Point points[4], int *stop);
...
void predictorCorrector(Point initial, Point points[4], FILE *f){
    ...
    // bit que controla si s'ha de seguir calculant la corba:
    int stop = 0;
    for(int index=0; index<POINTS_IN_CURVE; index++){
        
        // Calcula la predicció del punt següent
        X1 = predictor(X0, points, &stop);
        if(stop){
            // Si la predició ha fallat, parem de calcular la corba
            printf("Corba no completada perqùe el gradient era zero\n\n");
            break;
        }
        // Calcula la correcció del punt següent
        X2 = corrector(X1, X0, initial, points, &stop);
        if(stop){
            // Si la correcció ha fallat, parem de calcular la corba
            printf("Corba no completada perquè la correció no ha convergit\n\n");
            break;
        }
...
Point predictor(Point initial, Point points[4], int* stop){
    ...
    // Calcula el perpendicular al gradient
    Point perp = perpendicular(grad);
    // Comprova que el gradient no sigui zero
    if (fabs(perp.x) < TOL && fabs(perp.y) < TOL){
        // Si el gradient és zero, no es pot calcular la predicció i s'ha de deixar de calcular la corba
        *stop = 1;
...
*/
Point corrector(Point pred, Point centre, Point X0, Point points[4], int* stop){
    // Punt anterior
    Point X_prev = pred;
    ...
    // Es deixa de calcular aquesta corba
    *stop = 1;
    return (Point){0.0,0.0};
}
