...
void assignValues(double x[], double** u, double h, int n);
...
int gs(double** u, double x[], double a[9], int n, double error, int maxIter);
...
int main(){
    ...
    */
    ...
    double x[n+2];
    //vector solució en forma de matriu co a iteracio per gs
...
int gs(double** u, double x[], double a[9], int n, double error, int maxIter){
    ...
    do{
        ...
        for(int i=1; i<(n+1); i++){
            for(int j=1; j<(n+1); j++){
                current = u[i][j];
                u[i][j] = (g(x[i], x[j]) - a[0]*u[i-1][j-1] - a[1]*u[i][j-1] - a[2]*u[i+1][j-1] - a[3]*u[i-1][j] - a[5]*u[i+1][j] - a[6]*u[i-1][j+1] - a[7]*u[i][j+1] - a[8]*u[i+1][j+1])/a[4];
...
/*
Funció que calcula la norma infinit de 2 vectors (diferència) de dimensió n
*/
...
/*
Funció que calcula la norma infinit de dos vectors (diferència) escrits com a matriu nxn
n és la mida de la matriu
*/
double matrixInfNorm(double** m1, double** m2, int n){ 
